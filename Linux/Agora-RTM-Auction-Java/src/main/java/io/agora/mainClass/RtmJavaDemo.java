...
class CONFIG {
    ...
    public static final String REDIS_IP = "10.82.103.226";
    public static final String REDIS_KEY = "AUCTION_METADATA";
}

class ChannelListener implements RtmChannelListener {
    private String channel_;
    private RtmMetadata metadata_;
...
public class RtmJavaDemo {
    ...
    public void init() {
        try {
            ...
            mRtmClient = RtmClient.createInstance(CONFIG.APP_ID,
    ...
    public void setChannelMetadata() {
        ...
        while(true) {
            ...
            data = jedis.get(CONFIG.REDIS_KEY);

            if (data.length() == 0 || data.equals("nil"))
            {
                continue;
            }

            jedis.set(CONFIG.REDIS_KEY, "");
    ...
    public void sendChannelMetadata(String metadata) {
        /*mRtmChannel.getChannelMetadata(new ResultCallback<RtmMetadata>() {
            @Override
            public void onSuccess(RtmMetadata meta) {
                mRtmChannel.deleteChannelMetadata(meta.items, null, null);
            ...
            public void onFailure(ErrorInfo errorInfo) {
                final int errorCode = errorInfo.getErrorCode();
                System.out.println("getChannelMetadata failed, erroCode = "
                                + errorCode);
            }
        });
        */
        ...
        mRtmChannel.updateChannelMetadata(m.items, options, new ResultCallback<Void>() {
            @Override
            public void onSuccess(Void aVoid) {
            ...
            public void onFailure(ErrorInfo errorInfo) {
                final int errorCode = errorInfo.getErrorCode();
                System.out.println("Send Metadata to channel failed, erroCode = "
                                + errorCode);
            }
        });
    }
