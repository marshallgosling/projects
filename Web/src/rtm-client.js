...
export default class RTMClient extends EventEmitter {
  ...
  async sendChannelMetadata(metadata, options, channelName) {
    console.log('channelMetadata', metadata)
    if (!this.channels[channelName] || !this.channels[channelName].joined) return
    //let _m = this.channels[channelName].metadata
    //await this.channels[channelName].channel.deleteChannelMetadata([metadata], options)

    return this.channels[channelName].channel.addChannelMetadata([metadata], options)
  }
